...
ns = [[int(i) for i in s.split()] for s in triangle.split("\n") ]

def getMax(s, column, row, n=ns):
    if row >= len(n):
        return s
    return max(getMax(s + n[row][column], column, row + 1, n),
               getMax(s + n[row][column + 1], column + 1, row + 1, n))
